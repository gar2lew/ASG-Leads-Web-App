...
| R-008 | Emulator harnesses exist but callable mutation coverage is incomplete. | High | Preflight, Firestore smoke, authenticated rules harness, and callable dry-run harness now execute. Restore executable mutation coverage after Admin SDK timestamp compatibility is fixed. | Active |
...
| R-014 | Callable mutation paths using `admin.firestore.FieldValue.serverTimestamp()` fail in the Functions emulator runtime. | High | Add a targeted Functions Admin SDK compatibility goal. Do not deploy Functions until `setPin`, settings/admin mutation callables, and related timestamp-writing paths pass emulator coverage. | Active |
