...
| R-013 | Dependency advisories remain unresolved. Root `npm audit` reports 29 advisories, including 2 critical and 5 high. Functions `npm audit` reports 16 advisories, including 3 high. | Critical | Do not run broad audit fixes inside release work. Create a targeted dependency audit goal for `jspdf`, `vitest`, `vite`, Firebase packages, `firebase-admin`, and high-risk transitive Functions advisories. | Active |
