...
| R-013 | Dependency advisories remain unresolved. Root `npm audit` now reports 27 advisories, including 1 critical and 5 high, after the direct `jspdf` update to `4.2.1` and the lockfile-only `dompurify` update to `3.4.11`. Functions `npm audit` reports 16 advisories, including 3 high. Remaining risk includes direct `vitest` and `vite` dev-tooling advisories, Firebase web SDK transitive advisories, and Functions runtime transitive risk through Firebase and Google libraries. | Critical | Do not run broad audit fixes inside release work. Follow `docs/dependency-remediation-plan.md` with separate goals for Firebase web SDK, Functions runtime, and dev tooling. | Active |
