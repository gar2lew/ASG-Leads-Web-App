...
| R-013 | Dependency advisories remain unresolved. Root `npm audit` reports 29 advisories, including 2 critical and 5 high. Functions `npm audit` reports 16 advisories, including 3 high. The dependency security review confirms direct `jspdf` app runtime risk, direct `vite` and `vitest` dev-tooling risk, and Functions runtime transitive risk through Firebase and Google libraries. | Critical | Do not run broad audit fixes inside release work. Follow `docs/dependency-remediation-plan.md` with separate goals for PDF runtime, Firebase web SDK, Functions runtime, and dev tooling. | Active |
