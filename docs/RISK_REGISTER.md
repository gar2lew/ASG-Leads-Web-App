...
| R-024 | Functions emulator backend discovery can time out near the 10 second backend specification window, causing callable dry-run failures such as `functions/not-found` when callables do not register. | High | Use `docs/functions-emulator-startup-investigation.md`, Java 21 or newer, a warmed Firebase CLI, Node 22 alignment where possible, and `npm run test:emulator:preflight` before callable emulator checks. Do not change Firestore rules, emulator config, production Firebase, or dependencies as a shortcut. | Active |
